...
void plot(window<int> &scr, std::vector<int> &colors, int iter_max, const char *fname, bool smooth_color) {
    ...
    // Check if bitmap was successfully allocated
    if (!bitmap) {
        std::cerr << "Failed to allocate memory for the bitmap." << std::endl;
        #ifdef FREEIMAGE_LIB
    ...
    for (int i = scr.y_min(); i < scr.y_max(); ++i) {
        for (int j = scr.x_min(); j < scr.x_max(); ++j) {
            ...
            if (!FreeImage_SetPixelColor(bitmap, j, i, &col)) {
                std::cerr << "Failed to set pixel color at (" << j << ", " << i << ")." << std::endl;
            }
    ...
    if (!FreeImage_Save(FIF_PNG, bitmap, fname, PNG_DEFAULT)) {
